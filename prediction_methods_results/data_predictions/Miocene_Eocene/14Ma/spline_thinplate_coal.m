 ...
 nloop=5000;
 nwarmup=2000;
 data=readtable('miocene_v1.csv');
 data=table2array(data);
% load('var_names.mat');
 y_index=[4 5 6]
 %number_sites_deposit=sum(data(:,y_index),2);
 miss_index=find(data(:,y_index(1))==-999);
 nmiss=length(miss_index);
 ndep=length(y_index);
 ntot=size(data,1);
 y=data(:,y_index);%
 obs_index=find(number_sites_deposit~=0);
 ...
 Xlin(:,2:nvar+1)=(Xlin(:,2:nvar+1)-ones(ntot,1)*min(Xlin(:,2:nvar+1)))./(ones(ntot,1)*range(Xlin(:,2:nvar+1)));
...
csvwrite('deposit_results14Ma_v1.csv',results);
deposit_input=[Xspace deposit_input];
csvwrite('deposit_input14Ma_v1.csv',deposit_input)
