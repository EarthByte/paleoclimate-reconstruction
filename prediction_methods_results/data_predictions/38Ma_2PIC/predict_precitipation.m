 ...
 test_data=readtable('predictor_data_38Ma_land.csv');
 ...
 y_miss=exp(precip_test(:,y_index)).^.33;
 nmiss=length(y_miss);
 ntot=nmiss+nobs;
...
for j=1:ncont
    ...
    plot(xdata(:,2),ydata,'*')
    title(['Log Precipitation vs ',varnames(j+ndep,:)])
    %lsline
