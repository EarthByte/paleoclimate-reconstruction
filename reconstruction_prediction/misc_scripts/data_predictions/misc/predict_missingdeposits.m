...
function predict_missingdeposits(input_file, output_file, output_fileinput,accuracyfile)
...
%     classify_all(obs_index)=classify_nobs;
  %   classify_all(miss_index,k)=classify_miss;
...
%     up_cred_lim(miss_index)=up_cred_lim_miss;
 ...
 deposit_input =[Xspace coal_input evaporite_input glacial_input];
 
 results=[Xspace data(:,y_index)  coal_input evaporite_input glacial_input  coal_predict evaporite_predict glacial_predict  low_cred_lim(:,:) up_cred_lim];%writing results to file, first lat lon then deposit recording, the probability of a deposit(fit_hat), with lower and upper cred intervals
