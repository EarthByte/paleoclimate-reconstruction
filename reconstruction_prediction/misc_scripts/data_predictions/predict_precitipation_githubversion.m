function predict_precitipation_githubversion( input_file,input_file2, infile_deposit,  output_file) 
 state=sum(100*clock);
 rand('state', state);
 randn('state', state);
 nloop=5000;
 ...
 Xlin_14Ma_input=readtable(input_file);
 Xlin_14Ma_input=table2array(Xlin_14Ma_input);

 precip_train=readtable(input_file2);
 y_index=4;%log prec
 y_obs=table2array(precip_train(:,y_index));
 y_obs=y_obs.^0.33;
 nobs=length(y_obs);
 train_dep_index=[3 4 5];
 ndep=length(train_dep_index);
 train_cont_index=[7 9 ];
 ncont=length(train_cont_index);
 train_space_index=[2 3];
...
%  varnames=char(varnames);
 ...
 Xlin_obs=[ones(nobs,1) Xlin_14Ma_input(:,[2 3 4]);
...
results=[test_predictor_data(:,test_space_index) zeros(nmiss,1) miss_fit_hat low_cred_lim_miss up_cred_lim_miss]
