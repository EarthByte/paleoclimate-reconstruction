 ...
 space_index=[2,3];
 cont_index=[7 9 11 12];
 precip=table2array(precip);
 y_index=3;%
 y=precip(:,y_index).^0.33;
 ntot=length(y);
 ...
 nvar=size(Xlin,2)-1;
 Xlin(:,ndep+2:nvar+1)=(Xlin(:,ndep+2:nvar+1)-ones(ntot,1)*min(Xlin(:,ndep+2:nvar+1)))...
     ./(ones(ntot,1)*range(Xlin(:,ndep+2:nvar+1)));%scaling
