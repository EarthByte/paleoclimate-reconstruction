 ...
 data=datamissing;
 load('var_names.mat');
 y_index=8;%Glacial is the response variable
 ntot=size(data,1);
 y=table2array(data(:,y_index));%log precipitation
 nmiss=sum(isnan(y));
 miss_index=find(isnan(y)==1);
 obs_index=find(isnan(y)==0);
 y_obs=y(obs_index);
 nobs=length(y_obs);
 cont_xindex=[12 15];% column of continuous variabels other than lat and lon
 ...
 Xlin=table2array(data(:,lin_index));
 ...
 [omega_cov_obs_grid]=thinplate_basis(lat_obs,lon_obs,lat_grid,lon_grid);
 cov_invvar_miss_obs=omega_cov_obs_miss'/omega_obs;
 cov_invvar_grid_obs=omega_cov_obs_grid'/omega_obs;
...
%create storage space 
    ...
    lowlim(zero_index)=-1000;
         ...
         *(fit_obs-Xlin_obs*Beta_star(1:nvar+1));
           for t=1:nobs
               var_ftbar_obs(t)=zmat_obs(t,:)*post_beta_star_var*zmat_obs(t,:)';
           end
...
misclassify_rate=length(find((y_obs-classify_nobs)~=0))/nobs;
