...
class GPMC:
    ...
    def kernel_gp(self, kernelname = 'expsquared'):
        ...
        if kernelname == 'expsquared':
            k0 = 1. * kernels.ExpSquaredKernel([0.01,0.01], ndim = 2)  # + kernels.WhiteKernel(1.,ndim=2)
    ...
    def lnlike_gp(self, resid, p, sigma):
        ...
        try:
            ...
            if george.__version__ < '0.3.0':
                ...
                gplnlike = gp.lnlikelihood(resid, quiet=True)
            else:
                #gplnlike = gp.log_likelihood(self.residual_blr, quiet=True)
                gplnlike = gp.log_likelihood(resid, quiet=True)
        except:
    ...
    def lnprior_blr(self, beta, sigma):
        ...
        if np.any((0.01 > sigma) + (sigma > 2)):
            ...
            if(type=='uniform'):
                ...
                lnp_betas = -0.5*(fact1**2)/np.var(self.y)
                ln_prob = ln_prob + np.sum(lnp_betas)
            elif(type=='original'):
    ...
    def lnprob(self, params):
        ...
        #return lnp_blr + self.lnprior_gp(p) + self.lnlike_gp(resid_blr, p, sigma) 
        #return self.lnprior_blr(beta, sigma) + self.lnprior_gp(p) +  self.lnlike_gp(resid_blr, p, sigma) 
        return lnp_blr
